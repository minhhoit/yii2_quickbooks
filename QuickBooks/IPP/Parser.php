...
class QuickBooks_IPP_Parser
{
                ...
                case QuickBooks_IPP_IDS::OPTYPE_CDC:
                    ...
                    foreach ($List->children() as $ObjList) {
                        foreach ($ObjList->children() as $Child) {
                            ...
                            QuickBooks_Loader::load('/QuickBooks/IPP/Object/'.$Child->name().".php");
                            $Object = new $class();
                ...
                case QuickBooks_IPP_IDS::OPTYPE_QUERY:
                    ...
                    if (!array_key_exists('startPosition', $attrs) and
                        ...
                        foreach ($List->children() as $Child) {
                            ...
                            $class = 'minhhoit\yii2_quickbooks\QuickBooks_IPP_Object_'.$Child->name();
                            QuickBooks_Loader::load('/QuickBooks/IPP/Object/'.$Child->name().'.php');
    ...
    protected function _push($Node, $Object)
    {
        ...
        if ($Node->hasChildren()) {
            $class = 'minhhoit\yii2_quickbooks\QuickBooks_IPP_Object_' . $name;
            QuickBooks_Loader::load('/QuickBooks/IPP/Object/'.$name.".php");
            $Subobject = new $class;
